...
public class HarkkaRestTests {
	...
	public void statusOk() throws Exception {
		...
		System.out.println("HarkkaRestTest --- testing endpoint memosjson --- status ok");
		
		mockMvc.perform(get("/memosjson")).andExpect(status().isOk());

	}
	
	@Test
	public void responseTypeApplicationJson() throws Exception {
		System.out.println("HarkkaRestTest --- testing if endpoint /articlesjson has content type JSON --- status ok");
	...
	public void responseTypeApplicationJsonFail() throws Exception {		
		System.out.println("HarkkaRestTest --- testing if endpoint /mymemos has content type JSON --- FAIL");
		
		mockMvc.perform(get("/mymemos")) //fail koska vastaus ei ole jsonia
				.andExpect(content().contentType(MediaType.APPLICATION_JSON))
				//.andExpect(content().contentType(MediaType.APPLICATION_ATOM_XML_VALUE))
				.andExpect(status().isOk());
	...
	public void sbOneItemApiStatusOk() throws Exception {
		mockMvc.perform(get("/api/articles/3")).andExpect(status().isOk());
